...
// -------------------------------------------------------------------------------------------------- //
// ORB pattern
// 随机生成的点列，对于要进行比较的两幅图 需要使用同一组随机点列
...
void ComputeORB(const cv::Mat &img, //图像
		vector<cv::KeyPoint> &keypoints, //特征 Oriented FAST
		vector<DescType> &descriptors) { //输出 描述子
  const int half_patch_size = 8;
  const int half_boundary = 16;
  int bad_points = 0;
  for (auto &kp: keypoints) {//pt获取坐标
	//如果关键点在图片的外圈 16pixes 则直接排除 因为无法进行矩的计算
    ...
    for (int i = 0; i < 8; i++) {
      ...
      //k从0到31
      for (int k = 0; k < 32; k++) {
	      //idx_pq 从0到7*32+31=8*32-1=255 共256次
        int idx_pq = i * 32 + k;
	...
	//通过随机点列构建两位float的坐标
        ...
        // rotate with theta
	// steer BRIEF
	// 为了保持描述子的旋转不变性，引入FAST 计算的特征点方向
	// 通过旋转矩阵 旋转特征点和 p,q 的连线向量求出旋转后的 pp，qq 坐标 此时认为特征点为原点
	// 加上特征点坐标获取 pp，qq在 图像中的逻辑像素坐标
	// 逻辑像素坐标是float类型的 双线性插值？ 亚像素？
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
  ...
  for (size_t i1 = 0; i1 < desc1.size(); ++i1) {
	  //这里隐性的把 size_t 的 i1 转换成了 int 编译时会有警告
